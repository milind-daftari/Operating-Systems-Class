...
#define NUM_BUCKETS 5     // Buckets in hash table - Hash buckets are used to allocate data items for sorting or lookup purposes.
#define NUM_KEYS 100000   // Number of keys inserted per thread
int num_threads = 1;      // Number of threads (configurable)
int keys[NUM_KEYS];
pthread_mutex_t lock[NUM_BUCKETS]; // define locks
...
void insert(int key, int val) {
  ...
  pthread_mutex_lock(&lock[i]); // acquire locks
  ...
  e->val = val;
  table[i] = e;
  pthread_mutex_unlock(&lock[i]); // release locks
}
...
int main(int argc, char **argv) {
  ...
  // Initialize a mutex lock, return 1 if it fails
  for(i = 0; i< NUM_BUCKETS; i++)
  {
    thread_res = pthread_mutex_init(&lock[i], NULL); 
    if(thread_res != 0)
    {
      printf("\n mutex initialization has failed for %ld\n", i);
    }
  }
